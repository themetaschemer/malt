(module+ test
  ...
  (require (only-in (rename-in "A-scalar-ops.ss" (d-sqr sqr))
                    sqr))
  (require (only-in "../tensors.rkt" tensor ext2-ρ))
  ...
  (let ((a (tensor (tensor 3 4 5 6)
    ...
    (check-ρ-∇ (sse a b)
               (tensor 4 100)
               (list (tensor (tensor 2.0 2.0 2.0 2.0) (tensor 10.0 10.0 10.0 10.0))
  ...
  (let ((a (tensor (tensor 3 4 5)
                   (tensor 6 7 8))))
    (check-dual-equal? (d-sum-cols a) (tensor 9 11 13))
    (check-dual-equal? ((∇¹ (λ (b) (d-sum-cols (d* b b))))  a)
                       (list (tensor
                              (tensor 6.0 8.0 10.0)
                              (tensor 12.0 14.0 16.0)))))
  )
