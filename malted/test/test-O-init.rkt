(module+ test
  ...
  (define mean-v (force/eval (abs (/ (sum (sum v)) 4000))))
  (define variance-v (force/eval (- (/ (sum (sum (* v v))) 4000) (* mean-v mean-v))))
  ...
  (define mean-r (force/eval (abs (/ (sum (sum (sum r))) 8000))))
  (define variance-r (force/eval (- (/ (sum (sum (sum (* r r)))) 8000) (* mean-r mean-r))))
