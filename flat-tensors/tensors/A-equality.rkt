...
(define (equal-elements? actual expected)
  (let ((actual-offset (flat-offset actual))
    ...
    (and (equal? actual-size expected-size)
         (call/cc (λ (return)
                    (for/fold ([check #t]) ([i-actual (in-range actual-offset (+ actual-offset actual-size))]
                                            [i-expected (in-range expected-offset (+ expected-offset expected-size))])
                      (cond
                        (((equal-within-tolerance?)
                          (vector-ref actual-store i-actual)
                          (vector-ref expected-store i-expected)) check)
                        (else (return #f)))))))))
