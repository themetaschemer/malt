(module+ test
  ...
  (check-equal? (tref r1-td 2) 5.0)
  (check-equal? (tlen r1-td) 3)
  (check-equal? (list->tensor (list 3.0 4.0 5.0)) r1-td)

  (check-true (scalar? r0-td))
  (check-false (scalar? r1-td))

  (check-true (and (tensor? r0-td) (tensor? r1-td)))
  (check-false (tensor? '(a b c)))

  (check-equal? (built-tensor
                 (λ (idx)
                   (+ (* 6 (ref idx 0))
                      (* 2 (ref idx 1))
                      (ref idx 2)))
                 ...
                 '())
                r3-td)
  (check-equal? (build-tensor '(4 3 2)
                              (λ (idx)
                                (+ (* 6 (ref idx 0))
                                   (* 2 (ref idx 1))
                                   (ref idx 2))))
  ...
  (check-equal? (build-tensor '(1 2 3) (λ (idx) (+ (list-ref idx 0) (list-ref idx 1) (list-ref idx 2))))
                (tensor (tensor (tensor 0 1 2) (tensor 1 2 3))))

  (check-equal? (trefs r1-td '(0 2)) (tensor 3.0 5.0))
